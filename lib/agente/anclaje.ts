...
export function emptyEvidence(): Evidence {
  return { refs: new Set(), numbers: new Set(), customerNumbers: new Set() };
}
...
export type GroundingViolation = { kind: "reference" | "amount" | "quantity"; value: string };
...
export function checkGrounding(text: string, ev: Evidence): { ok: boolean; violations: GroundingViolation[] } {
  ...
  for (const m of text.toUpperCase().matchAll(REF)) if (!ev.refs.has(m[1])) add({ kind: "reference", value: m[1] });
  for (const m of text.matchAll(PESOS)) {
    const n = toInt(m[1]);
    if (!ev.numbers.has(n)) add({ kind: "amount", value: m[0].trim() }, n);
  }
  for (const m of text.matchAll(MILES)) {
    const n = toInt(m[0]);
    if (!ev.numbers.has(n)) add({ kind: "amount", value: m[0] }, n);
  }
  for (const m of text.matchAll(MIL)) {
    const n = Math.round(Number(m[1].replace(",", ".")) * 1000);
    if (!ev.numbers.has(n)) add({ kind: "amount", value: m[0].trim() }, n);
  }
  for (const m of text.matchAll(UNIDADES)) {
    const n = Number(m[1]);
