...
describe("Fase 1 — Blocker #7: enrutador de intenciones (motor, sin IA real)", () => {
  ...
  it("9. '¿Cuánto cuesta?' -> clasificada como OTRO -> termina SIN ningún mensaje (deja pasar a LEGACY)", () => {
  ...
  it("11. mensaje ambiguo -> clasificación 'otro' explícita -> termina sin mensaje", () => {
    const r = clasificarComoConEfectos(flow, "no sé qué servicio hacerme", "otro");
    assert.equal(r.state.currentNodeId, "end-otro");
  });

  it("12. mensaje vacío -> __firstMessageText nunca se siembra (Blocker #1), y si la clasificación no llega (data vacía) el motor usa la rama default -> termina sin mensaje, nunca asume 'agendar'", () => {
    let state = createFlowEngineState(flow, { executionId: randomUUID() });
    ...
    assert.equal(r.state.currentNodeId, "end-otro", "sin clasificación reconocible, cae al default -- NUNCA asume agendar por defecto");
  });
...
describe("Fase 1 — Blocker #7: cableado real del hand-off a LEGACY (sin Claude, sin DB)", () => {
  it("clasificación 'otro' -> decidirFallbackDesdeResultado (lógica REAL de decisión, la misma que usa route.ts) dice handled=false", () => {
    const flow = danielaRouterFlow();
    let state = createFlowEngineState(flow, { executionId: randomUUID() });
    ...
    assert.equal(decision.handled, false, "el router debe ceder el turno a LEGACY para preguntas de información");
