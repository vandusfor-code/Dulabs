...
describe("resolverFechaSolicitada — respaldo: la propuesta de la IA solo si es una fecha real y no pasada", () => {
  it("texto no reconocible + propuesta válida futura => se acepta (origen propuesta)", () => {
    assert.deepEqual(resolverFechaSolicitada({ solicitudTexto: "cuando puedas", fechaPropuesta: "2030-03-20", hoyISO: HOY }), { ok: true, fecha: "2030-03-20", origen: "propuesta" });
    assert.deepEqual(resolverFechaSolicitada({ fechaPropuesta: HOY, hoyISO: HOY }), { ok: true, fecha: HOY, origen: "propuesta" });
  });

  it("propuesta en el pasado (típico: la IA supone otro año) => fecha_pasada", () => {
    assert.deepEqual(resolverFechaSolicitada({ solicitudTexto: "cuando puedas", fechaPropuesta: "2024-05-04", hoyISO: HOY }), { ok: false, motivo: "fecha_pasada" });
  });
