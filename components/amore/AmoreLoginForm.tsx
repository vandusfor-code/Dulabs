...
// Login AMORE (autorizado) — formulario compartido entre las dos entradas
// reales del panel: /amore/login (móvil, /agenda/[token] -- sin tocar) y
// /amoreweb/login (escritorio, migrado desde /admin/amore, autorizado).
// Extraído de la página original para que ninguna de las dos rutas
// duplique el formulario/lógica de llamada a /api/agenda-auth/login --
// cada página solo decide A DÓNDE redirigir tras el éxito (`onSuccess`).
export function AmoreLoginForm({ onSuccess }: { onSuccess: (token: string) => void }) {
