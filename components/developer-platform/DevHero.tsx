...
// DuLabs Developer V1 -- Fase 15 / rediseño Resend-level. Hero: el producto es
// el protagonista. En vez de 4 tarjetas de estado apiladas, muestra un
// request/response REAL de la API (mismo contrato que /developers y el gateway)
// + el ciclo de vida compacto del mensaje. Nomenclatura real: POST
// /api/v1/messages, Bearer dl_live_, Idempotency-Key, jobId/status.
      ...
      <div className="mx-auto grid max-w-[1440px] items-center gap-14 px-6 lg:grid-cols-[1fr_1.05fr]">
        ...
        <div className="max-w-xl">
          <p className="inline-flex items-center gap-2 rounded-full border border-site-border bg-site-card px-3 py-1 font-mono text-[11px] uppercase tracking-[0.2em] text-site-muted-fg">
            <span className="h-1.5 w-1.5 rounded-full bg-site-fg" />
        ...
        {/* Producto: request real -> response -> ciclo de vida */}
        <div className="relative">
          <div className="overflow-hidden rounded-2xl border border-site-border bg-site-card">
            ...
            <pre className="overflow-x-auto bg-site-bg px-4 py-4 font-mono text-[11.5px] leading-relaxed text-site-fg"><code>{REQUEST}</code></pre>
            ...
            <div className="flex items-center gap-1.5 border-t border-site-border px-4 py-3 font-mono text-[11px]">
              {LIFECYCLE.map((s, i) => (
                <span key={s} className="inline-flex items-center gap-1.5">
                  <span className={i === LIFECYCLE.length - 1 ? "text-site-fg" : "text-site-muted-fg"}>{s}</span>
              ...
              <span className="ml-auto inline-flex items-center gap-1.5 text-[10px] uppercase tracking-widest text-site-muted-fg">
                <span className="h-1.5 w-1.5 rounded-full bg-site-fg" /> webhook
              </span>
            </div>
          </div>
        </div>
