...
import { Caption, HomeSection, SectionHeader, TARJETA_HOVER } from "./atoms";
...
export function CapabilitiesSection() {
  return (
    <HomeSection id="capacidades" titleId="capacidades-titulo">
      ...
      <ul className="mt-8 grid border-t border-site-border sm:mt-10 sm:gap-3 sm:border-t-0 sm:grid-cols-2 lg:grid-cols-3 lg:gap-4 xl:grid-cols-4 xl:mt-12">
        {CAPACIDADES_HOME.map((c) => {
          const Icono = ICONOS[c.id];
          return (
            <li
              key={c.id}
              className={`border-b border-site-border py-4 last:border-b-0 sm:rounded-xl sm:border sm:border-b sm:border-site-border/70 sm:bg-site-card/50 sm:p-5 sm:last:border-b xl:p-6 ${TARJETA_HOVER}`}
            >
