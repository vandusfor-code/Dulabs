...
type Kind = "loading" | "not_found" | "error" | "no_versions";
...
const CONFIG: Record<Kind, { icon: LucideIcon; spin?: boolean; title: string; body: string }> = {
  ...
  no_versions: {
    icon: PackageOpen,
    title: "Este Flow todavía no tiene versiones",
    body: "Crea una versión (POST /api/flows/[id]/versions) para poder visualizarlo aquí.",
  },
};

/** Etapa 1 (Flow Builder, autorizado) — estados no-cargados, sin pantallas genéricas. */
export function FlowStateScreen({ kind, message }: { kind: Kind; message?: string }) {
