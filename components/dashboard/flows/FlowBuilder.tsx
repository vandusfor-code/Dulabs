...
import { ensureInitialVersion, duplicateFlow as duplicateFlowRequest } from "@/lib/flow-builder/create-flow";
...
// F15.1 (Admin Flow Studio, autorizado) -- recuperación de draft local. Red
// de seguridad ADICIONAL al autosave del servidor (que ya existe vía
// saveFlowVersion): si la pestaña se cierra/recarga/pierde conexión antes de
// que el autosave al servidor termine, esto permite recuperar exactamente lo
// que había en el canvas. NUNCA es la fuente de verdad -- eso sigue siendo el
// servidor -- por eso un fallo de localStorage (modo privado, cuota llena) se
// ignora en silencio en vez de romper el guardado real.
const FLOW_DRAFT_STORAGE_PREFIX = "dulabs:flow-studio-draft:";

interface StoredFlowDraft {
  definition: FlowDefinition;
  savedAt: string;
}

function draftStorageKey(flowId: string): string {
  return `${FLOW_DRAFT_STORAGE_PREFIX}${flowId}`;
}

function readStoredDraft(flowId: string): StoredFlowDraft | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(draftStorageKey(flowId));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<StoredFlowDraft>;
    if (!parsed || typeof parsed !== "object" || !parsed.definition || !parsed.savedAt) return null;
    return { definition: parsed.definition, savedAt: parsed.savedAt };
  } catch {
    return null;
  }
}

function writeStoredDraft(flowId: string, definition: FlowDefinition) {
  if (typeof window === "undefined") return;
  try {
    const draft: StoredFlowDraft = { definition, savedAt: new Date().toISOString() };
    window.localStorage.setItem(draftStorageKey(flowId), JSON.stringify(draft));
  } catch {
    // ver comentario de arriba -- el draft local es best-effort.
  }
}

function clearStoredDraft(flowId: string) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem(draftStorageKey(flowId));
  } catch {
    // ver comentario de arriba -- el draft local es best-effort.
  }
}
...
export interface FlowBuilderProps {
  flowId: string;
  /**
   * F15.1 (Admin Flow Studio, autorizado) -- presente SOLO cuando un admin
   * de DuLabs está operando el Flow de un cliente vía /flow-studio/* (ver
   * lib/flow/api-auth.ts::requireFlowAccess con allowAdminOverride). Se
   * hilvana hasta el último wrapper de lib/flow-builder/*.ts -- nunca se usa
   * para decidir nada de autorización acá (eso lo hace el backend); solo
   * viaja como dato para que el backend resuelva el tenant correcto.
   */
...
export function FlowBuilder({ flowId, adminTenantId, adminTenantName, colorMode = "light", backHref = "/dashboard/flows" }: FlowBuilderProps) {
  ...
  // F15.1 (Admin Flow Studio, autorizado) -- draft local pendiente de
  // recuperación (ver readStoredDraft/writeStoredDraft arriba). Solo se
  // llena cuando, tras cargar la definición del servidor, existe un draft en
  // localStorage que DIFIERE de lo que el servidor devolvió -- nunca se
  // aplica automáticamente, siempre requiere una acción explícita
  // (Recuperar/Descartar) para no pisar silenciosamente un guardado real.
  const [pendingDraft, setPendingDraft] = useState<StoredFlowDraft | null>(null);
  ...
  // F15.1 (Admin Flow Studio, autorizado) -- Duplicar Flow.
  ...
  useEffect(() => {
    ...
    async function cargar() {
      const headers = flowApiHeaders(session!.access_token, { adminTenantId });
      ...
      if (loadResult.kind === "loaded") {
        ...
        // F15.1 -- si hay un draft local (misma pestaña u otra) que difiere
        // de lo que el servidor acaba de devolver, se ofrece recuperarlo en
        // vez de aplicarlo solo (ver comentario de StoredFlowDraft arriba).
        const draft = readStoredDraft(flowId);
        setPendingDraft(draft && JSON.stringify(draft.definition) !== JSON.stringify(definition) ? draft : null);
      } else {
        setEditorState(null);
  ...
  const intentarPrepararFlow = useCallback(async () => {
    ...
    const res = await ensureInitialVersion({ flowId, accessToken: session.access_token, adminTenantId });
    if (res.ok) {
  ...
  }, [session, flowId, result, adminTenantId]);
  ...
  const cargarIntegraciones = useCallback(() => {
    if (!session) return;
    listIntegrations({ accessToken: session.access_token, adminTenantId }).then((resultado) => {
      if (resultado.ok) setIntegrations(resultado.integrations);
    });
  }, [session, adminTenantId]);
  ...
  useEffect(() => {
    if (!session) return;
    listAgentesDelTenant({ accessToken: session.access_token, adminTenantId }).then((resultado) => {
  ...
  // F15.1 (Admin Flow Studio, autorizado) -- guarda el draft en localStorage
  // en cada edición (debounced), como red de seguridad ADICIONAL al autosave
  // al servidor de más abajo. Deliberadamente NO limpia el draft cuando
  // isDirty pasa a false por sí solo (eso pisaría, en el primer render tras
  // cargar, un draft recién detectado antes de que la clienta decida
  // Recuperar/Descartar) -- el draft solo se limpia explícitamente: al
  // guardar con éxito (handleSave), al descartar cambios (handleDiscard) o
  // al descartar el draft ofrecido (handleDiscardDraft).
  useEffect(() => {
    if (!flowId || !builderState?.isDirty) return;
    const definitionToStore = builderState.definition;
    const timer = setTimeout(() => {
      writeStoredDraft(flowId, definitionToStore);
    }, 800);
    return () => clearTimeout(timer);
  }, [flowId, builderState?.definition, builderState?.isDirty]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    listTriggers({ flowId, accessToken: session.access_token, adminTenantId }).then((res) => {
      if (cancelled) return;
  ...
  }, [session, flowId, result.kind, adminTenantId]);
  ...
  async function handleDeleteTrigger(trigger: FlowTrigger) {
    ...
    const result = await deleteTrigger({ flowId, triggerId: trigger.id, accessToken: session.access_token, adminTenantId });
  ...
  async function handleToggleTriggerEnabled(trigger: FlowTrigger) {
    if (!session) return;
    const result = await updateTrigger({ flowId, triggerId: trigger.id, enabled: !trigger.enabled, accessToken: session.access_token, adminTenantId });
  ...
  async function handleSubmitTrigger(input: TriggerModalSubmit) {
    ...
    const result =
      triggerModal.mode === "create"
        ? await createTrigger({ flowId, config: input.config, priority: input.priority, enabled: input.enabled, accessToken: session.access_token, adminTenantId })
        : await updateTrigger({
            flowId,
  ...
  function handleDiscard() {
    ...
    setJustPublished(false);
    setPublishRequestError(null);
    clearStoredDraft(flowId);
  }

  // F15.1 (Admin Flow Studio, autorizado) -- aplica el draft local ofrecido
  // por el banner de recuperación. Reusa edit() (no una asignación directa a
  // editorState) para que quede empujado al historial de undo/redo como
  // cualquier otra edición, y para que se invaliden justSaved/justPublished
  // igual que con cualquier cambio.
  function handleRecoverDraft() {
    if (!pendingDraft) return;
    edit(() => pendingDraft.definition);
    setPendingDraft(null);
  }
  ...
  async function handleSave() {
    ...
    if (result.ok) {
      ...
      setJustSaved(true);
      // F15.1 -- ya quedó guardado en el servidor (fuente de verdad), el
      // draft local pierde su propósito.
      clearStoredDraft(flowId);
  ...
  async function refreshVersions() {
    ...
    setVersionsError(null);
    const result = await fetchFlowVersions({ flowId, accessToken: session.access_token, adminTenantId });
  ...
  async function handlePublish() {
    ...
    setPublishing(true);
    setPublishRequestError(null);
    const result = await publishFlowVersion({ flowId, versionId, accessToken: session.access_token, adminTenantId });
    setPublishing(false);
    if (result.ok) {
  ...
  // F15.1 (Admin Flow Studio, autorizado) -- Duplicar: crea un Flow nuevo
  // dentro del MISMO tenant con la última definición guardada de este
  // (lib/flow/flow-store.ts::duplicateFlow, vía POST /api/flows/[id]/duplicate)
  // y navega al Studio del Flow nuevo. window.prompt (mismo patrón nativo ya
  // usado en este módulo para confirmaciones simples, ver handleDeleteTrigger)
  // -- pide el nombre, nunca duplica en silencio.
  async function handleDuplicate() {
    if (!session || duplicating) return;
    const nombreSugerido = `${flow.name} (copia)`;
    const nombre = window.prompt("Nombre del Flow duplicado:", nombreSugerido);
  ...
  // F15.1 (Admin Flow Studio, autorizado) -- AUTOSAVE. El Flow ya no depende
  // únicamente del botón "Guardar": tras una pausa real en la edición
  // (debounce, nunca en cada cambio) se guarda solo, reutilizando handleSave
  // TAL CUAL (mismo saveStatus/SavePill de siempre -- "Guardando…"/
  // "Guardado"/"Error al guardar" ya cubren exactamente los estados que
  // pide el autosave, sin UI nueva). Un ref siempre apunta a la versión más
  // reciente de handleSave para que el timer nunca capture un closure viejo
  // sin tener que re-armar el debounce en cada render. Este efecto (como
  // todos los hooks de este componente) corre ANTES del return condicional
  // de más abajo -- nunca viola las Rules of Hooks.
  const handleSaveRef = useRef(handleSave);
  useEffect(() => {
    handleSaveRef.current = handleSave;
  });
  useEffect(() => {
    if (!builderState?.isDirty || !canSave || saving) return;
    const AUTOSAVE_DEBOUNCE_MS = 2500;
    const timer = setTimeout(() => {
      void handleSaveRef.current();
    }, AUTOSAVE_DEBOUNCE_MS);
    return () => clearTimeout(timer);
  }, [builderState?.definition, builderState?.isDirty, canSave, saving]);
    ...
    return (
      <div className="flex flex-col">
        <header className="flex items-center gap-4 border-b border-edge bg-card px-5 py-3">
          <Link href={backHref} className="flex items-center gap-1.5 text-sm text-mist hover:text-fg">
            <ArrowLeft className="size-4" />
            Flows
          </Link>
          <div className="h-5 w-px bg-edge" />
          <h1 className="text-sm font-semibold text-fg">
            Flow Builder{adminTenantName ? ` · ${adminTenantName}` : ""}
          </h1>
        </header>
  ...
  return (
    <div className="flex flex-col">
      <FlowTopbar
        ...
        backHref={backHref}
        contextLabel={adminTenantName}
        canDuplicate={canSave}
        onDuplicate={handleDuplicate}
        duplicating={duplicating}
      />
      {duplicateError && (
        <div className="border-b border-edge bg-red-500/10 px-5 py-2 text-xs text-red-400">{duplicateError}</div>
      )}
      {pendingDraft && (
        <div className="flex items-center justify-between gap-3 border-b border-edge bg-sky-400/10 px-5 py-2 text-xs text-sky-400">
          <span>Se encontró un borrador sin guardar de {new Date(pendingDraft.savedAt).toLocaleString("es-CO")}.</span>
          <div className="flex shrink-0 items-center gap-3">
            <button
              type="button"
              onClick={handleRecoverDraft}
              className="font-medium underline underline-offset-2 hover:text-sky-300"
            >
              Recuperar
            </button>
            <button
              ...
              className="font-medium text-mist underline underline-offset-2 hover:text-fg"
            >
              Descartar
            </button>
          </div>
