...
export function CodeSample({ curl, js, titulo }: { curl: string; js?: string; titulo?: string }) {
  ...
  return (
    <div className="my-4 overflow-hidden rounded-lg border border-edge bg-ink">
      <div className="flex items-center justify-between border-b border-edge px-3 py-2">
        <div className="flex items-center gap-1">
          ...
          {titulo ? <span className="ml-2 text-xs text-mist">{titulo}</span> : null}
        </div>
        <button onClick={copiar} className="rounded border border-edge px-2 py-0.5 text-xs text-mist hover:text-fg">{copiado ? "Copiado" : "Copiar"}</button>
      </div>
