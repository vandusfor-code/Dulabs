...
-- ============================================================
-- 2. CREAR MIEMBRO con límite de cuenta (miembros distintos)
-- ============================================================
-- Cuenta usuarios DISTINTOS across los workspaces de la cuenta. Re-agregar a
-- un usuario que YA es miembro de la cuenta no consume cupo (solo ajusta rol).
-- p_limite_miembros NULL = sin tope. Devuelve el id de la membresía.
...
begin
  ...
  if v_account_id is not null then
    ...
    -- ¿el usuario ya es miembro de ALGÚN workspace de la cuenta?
    select count(*) into v_ya_es
      from public.dulabs_dev_memberships m
      join public.dulabs_dev_workspace_plans wp on wp.workspace_id = m.workspace_id
     where wp.account_id = v_account_id and m.user_id = p_user_id;
    if p_limite_miembros is not null and v_ya_es = 0 then
    ...
    -- Legacy sin cuenta: límite por-workspace (compat con comportamiento previo).
    perform pg_advisory_xact_lock(hashtextextended('ws:mem:' || p_workspace_id::text, 0));
    select count(*) into v_ya_es from public.dulabs_dev_memberships m
