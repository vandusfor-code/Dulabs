...
M = [
    ...
    ("M11 ubicación en la dirección pasa a una asesora", "lib/agente/runtime.ts",
     "    if (ckStep && pideEscrito) {", "    if (false) {", LG),
    ...
    # --- Auditoría final (lenguaje humano real) ---
    ("M18 el modelo inventa la cantidad", "lib/agente/herramientas.ts",
     "          if (!stated) return fail(\"QUANTITY_NOT_STATED\"", "          if (false) return fail(\"QUANTITY_NOT_STATED\"", LG),
    ("M19 'los dos' acepta solo uno", "lib/agente/herramientas.ts",
     "          if (incompleto) return fail(", "          if (false) return fail(", LG),
    ("M20 'listo' falso tras un bloqueo del carrito", "lib/agente/anclaje.ts",
     "  if (ev.cartBlocked && !ev.cartWritten && CARRITO_CLAIM.test(withoutUrls))", "  if (false && ev.cartBlocked && !ev.cartWritten && CARRITO_CLAIM.test(withoutUrls))", LG),
    ("M21 'el de la foto' no se resuelve con una sola foto", "lib/agente/seleccion.ts",
     "fotos.length === 1 && ", "false && ", LG),
    ("M22 'calle 20' cuenta como dirección completa", IN,
     "    if (grupos >= 2 || rural || (grupos >= 1 && lugar)) return { tipo: \"ok\", valor };\n    return { tipo: \"vaga\" };", "    return { tipo: \"ok\", valor };", LG),
    ("M23 'vivo en Montería' / 'el dorado' como nombre", "lib/agente/lenguaje/lexico.ts",
     "  \"vivo\", \"vive\", \"vivimos\",", "  \"vive\", \"vivimos\",", LG),
    ("M24 ciudad 'vivo en Montería' sin quitar el prefijo", IN,
     "[\"vivo en\", \"estoy en\",", "[\"estoy en\",", LG),
    ("M25 estado inventado: respuesta genérica en vez del estado real", "lib/agente/runtime.ts",
     "          ? realOrderStatus(activeOrder)", "          ? null", LG),
    ("M26 'y si lo recojo' sin signo cambia la entrega", "lib/agente/lenguaje/lexico.ts",
